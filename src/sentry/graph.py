...
def build_graph(
    *,
    ...
    graph.add_node(
        "critique", with_span("critique", make_critique_node(llm, memory=memory))
    )
    graph.add_node(
        "write_memory",
        with_span("write_memory", make_write_memory_node(memory)),
    )
    ...
    graph.add_edge("critique", "write_memory")
    graph.add_edge("write_memory", "format_review")
